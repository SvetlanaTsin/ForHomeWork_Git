...
1. Информацию из клона "Стороннего репозитория", который был создан в моем аккаунте на GitHub загружаем в локальный репозиторий *(см. Раздел 2, п. 2,3)*. Содаем новую ветку, изменяем ее. 
  
3. Загружаем информацию из локального репозитория в клон "Стороннего репозитория", который был создан в Моем аккаунте GitHub. Для этого используем команду *git push*. Git сам предложит правильный вариант команды *git push* с указанием папки. Копируем ее в командную строку. 

4. Предлагаем владельцу "Стороннего репозитория" "перетянуть" к себе изменения или дополнения, которые мы сделали. 
